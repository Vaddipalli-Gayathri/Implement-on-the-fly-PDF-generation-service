...
RUTA_TEMPORAL = os.path.join(BASE_DIR, 'static', 'temp')
output_pdf = os.path.join(BASE_DIR, 'static', 'pdf', 'pdf_generado.pdf')
rutacss = os.path.join(BASE_DIR, 'static', 'css', 'stylePlantilla.css')
# ---------------------------------------------------------------------------
...
def generarPdf():
    ...
    if request.method == 'POST':
        ...
        if(request.files['archivo'] and extensiones_validas(imagen.filename)):
            ...
            imagen = os.path.join(RUTA_TEMPORAL, nombre_archivo)
        else:
